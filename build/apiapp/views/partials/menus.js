...
KISSY.add('apiapp/views/partials/menus', function(S, View, MM, Mustache, Magix) {
     return View.extend({template:"core list:<br  /> {{#coreList}} <a href=\"#!/{{infos.loader}}/{{infos.ver}}/{{name}}\">{{dName}}</a><br  /> {{/coreList}} ext list:<br  /> {{#extList}} <a href=\"#!/{{infos.loader}}/{{infos.ver}}/{{name}}\">{{dName}}</a><br  /> {{/extList}}",
        render: function() {
            var me = this;
